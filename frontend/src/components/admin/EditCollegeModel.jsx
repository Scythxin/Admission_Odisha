...
export default function EditCollegeModel({ college, onClose, onUpdated }) {
  ...
  useEffect(() => {
    fetchWithAuth(`${API_BASE}?r=dashboard/list-media&type=banners`)
      ...
      .then(json => {
        if (json.status === "success") {
          setAvailableBanners(json.data.filter(item => !item.is_dir));
        }
      })
      .catch(console.error);
    ...
    fetchWithAuth(`${API_BASE}?r=dashboard/list-media&type=colleges`)
      ...
      .then(json => {
        if (json.status === "success") {
          setAvailableImages(json.data.filter(item => !item.is_dir));
        }
      })
      .catch(console.error);
  }, []);
  ...
  function handleSubmit(e) {
    ...
    setSubmitting(true);
    setError(null);
    fetchWithAuth(`${API_BASE}?r=dashboard/update-college`, {
      method: "POST",
                  ...
                  <div>
                    ...
                    >
                      ...
                      {availableImages.map((img) => (
                        <option key={img.filename} value={img.url}>{img.filename}</option>
                      ))}
                    </select>
                    {form.image && (
                      <div className="mt-2">
